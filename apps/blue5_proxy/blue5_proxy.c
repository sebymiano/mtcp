...
static void XorPacketPayload(char *buf, int buf_size) {
...
ForwardSocketData(struct thread_context *ctx, int src_sock, int dst_sock)
{
	...
	rd = mtcp_read(ctx->mctx, src_sock, buf, MAX_BUF_SIZE);
	if (rd <= 0) {
		return rd;
	}

	XorPacketPayload(buf, rd);
		...
		for (i = 0; i < nevents; i++) {
			...
			if (events[i].data.sockid == listener) {
			    ...
			    if (sv->is_server_socket) {
			      ...
			      TRACE_APP("Close request from socket %d (server), closing all clients' sockets\n",
...
//                  }
			      ...
			      CloseConnection(ctx, events[i].data.sockid);
                    ...
                    TRACE_APP("Close request from socket %d (client)\n", events[i].data.sockid);
                    CloseConnection(ctx, events[i].data.sockid);
                }
